...
REAL fhit1, fhit3, fhit10, fmr, fmrr;
REAL hit1TC, hit3TC, hit10TC, mrTC, mrrTC;
REAL fhit1TC, fhit3TC, fhit10TC, fmrTC, fmrrTC;
...
//            (l_filter_reci_rank+r_filter_reci_rank)/2, (l_filter_rank+r_filter_rank)/2, (l_filter_tot+r_filter_tot)/2, (l3_filter_tot+r3_filter_tot)/2, (l1_filter_tot+r1_filter_tot)/2);
    ...
    mr = (l_rank+r_rank)/2;
    hit10 = (l_tot+r_tot)/2;
    hit3 = (l3_tot+r3_tot)/2;
    hit1 = (l1_tot+r1_tot)/2;
    fmrr = (l_filter_reci_rank+r_filter_reci_rank) / 2;
    fmr = (l_filter_rank+r_filter_rank) / 2;
    fhit10 = (l_filter_tot+r_filter_tot) / 2;
    fhit3 = (l3_filter_tot+r3_filter_tot) / 2;
    fhit1 = (l1_filter_tot+r1_filter_tot) / 2;
//    printf("%f\n",l_tot);
//    printf("%f\n",hit10);
...
//                (l_filter_reci_rank_constrain+r_filter_reci_rank_constrain)/2, (l_filter_rank_constrain+r_filter_rank_constrain)/2, (l_filter_tot_constrain+r_filter_tot_constrain)/2, (l3_filter_tot_constrain+r3_filter_tot_constrain)/2, (l1_filter_tot_constrain+r1_filter_tot_constrain)/2);
        ...
        mrrTC = (l_reci_rank_constrain+r_reci_rank_constrain)/2;
        mrTC = (l_rank_constrain+r_rank_constrain)/2;
        hit10TC = (l_tot_constrain+r_tot_constrain)/2;
        hit3TC = (l3_tot_constrain+r3_tot_constrain)/2;
        hit1TC = (l1_tot_constrain+r1_tot_constrain)/2;
        fmrrTC = (l_filter_reci_rank_constrain+r_filter_reci_rank_constrain)/2;
        fmrTC = (l_filter_rank_constrain+r_filter_rank_constrain) / 2;
        fhit10TC = (l_filter_tot_constrain+r_filter_tot_constrain) / 2;
        fhit3TC = (l3_filter_tot_constrain+r3_filter_tot_constrain) / 2;
        fhit1TC = (l1_filter_tot_constrain+r1_filter_tot_constrain) / 2;
    }
}
...
extern "C"
REAL testHeadH(INT lastHead) {
    INT h = testList[lastHead].h;
    return h;
}

extern "C"
REAL testHeadT(INT lastHead) {
    INT t = testList[lastHead].t;
    return t;
}

extern "C"
REAL testHeadR(INT lastHead) {
    INT r = testList[lastHead].r;
    return r;
}

extern "C"
REAL  getTestLinkL_mrr(bool type_constrain = false) {
    if (type_constrain)
        return l_reci_rank_constrain;
    return l_reci_rank;
}
extern "C"
REAL  getTestLinkR_mrr(bool type_constrain = false) {
    if (type_constrain)
        return r_reci_rank_constrain;
    return r_reci_rank;
}
extern "C"
REAL  getTestLinkL_fmrr(bool type_constrain = false) {
    if (type_constrain)
        return l_filter_reci_rank_constrain;
    return l_filter_reci_rank;
}
extern "C"
REAL  getTestLinkR_fmrr(bool type_constrain = false) {
    if (type_constrain)
...
extern "C"
REAL  getTestLinkL_rank(bool type_constrain = false) {
    if (type_constrain)
        return l_rank_constrain;
    return l_rank;
}
extern "C"
REAL  getTestLinkR_rank(bool type_constrain = false) {
    if (type_constrain)
        return r_rank_constrain;
    return r_rank;
}
extern "C"
REAL  getTestLinkL_filter_rank(bool type_constrain = false) {
    if (type_constrain)
        return l_filter_rank_constrain;
    return l_filter_rank;
}
extern "C"
REAL  getTestLinkR_filter_rank(bool type_constrain = false) {
    if (type_constrain)
        return r_filter_rank_constrain;
...
extern "C"
REAL getTestLinkFHit10(bool type_constrain = false) {
    if (type_constrain)
        return hit10TC;
//    printf("%f\n", fhit10);
    return fhit10;
...
extern "C"
REAL  getTestLinkFHit3(bool type_constrain = false) {
    if (type_constrain)
        return fhit3TC;
    return fhit3;
}
...
extern "C"
REAL  getTestLinkFHit1(bool type_constrain = false) {
    if (type_constrain)
        return fhit1TC;
    return fhit1;
}
...
extern "C"
REAL  getTestLinkFMR(bool type_constrain = false) {
    if (type_constrain)
        return fmrTC;
    return fmr;
}
...
extern "C"
REAL  getTestLinkFMRR(bool type_constrain = false) {
    if (type_constrain)
        return fmrrTC;
    return fmrr;
}
...
void getNegTest() {
    ...
    for (INT i = 0; i < testTotal; i++) {
        ...
        if (randd(0) % 1000 < 500)
            negTestList[i].t = corrupt_head_train(0, testList[i].h, testList[i].r);
        else
            negTestList[i].h = corrupt_tail_train(0, testList[i].t, testList[i].r);
    }
}
