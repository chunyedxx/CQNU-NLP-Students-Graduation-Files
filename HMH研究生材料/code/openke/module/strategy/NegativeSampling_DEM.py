...
class NegativeSampling_DEM(Strategy):
	...
	def forward(self, data):
		...
		# print(score.shape[0])
			...
			batch_size = int(len(data['batch_h']) / (self.neg_ent + 1))
			regul,score = self.model.Regularization(data,batch_size)
			p_score = self._get_positive_score(score)
			n_score = self._get_negative_score(score)
			# p_regul = self._get_positive_score(regul)
			# n_regul = self._get_negative_score(regul)
			loss_res = self.loss(p_score, n_score)
			# loss_res -= self.regul_rate * regul.mean()
			loss_res -= self.regul_rate * regul.mean()
			# loss_res += self.regul_rate * n_regul.mean()
